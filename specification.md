...
Every processed image produces two output files stored in parallel directories within SharePoint (local path in development). Images are organized by content category within each storage tier.
...
**High-Res** preserves the original pixel data exactly — no resize, no re-encode. The source format is retained (JPEG, PNG, etc.). Files are named `{slug}-original.{ext}`.
...
The `Location` Airtable field stores the category-relative WebP path (e.g. `Headshots/proxima-mike7.webp`). The app constructs full SharePoint paths as:
...
| `High-Res Location` | Text | Original path relative to `Images/High-Res/` (e.g. `Headshots/proxima-mike7-original.jpg`) |
